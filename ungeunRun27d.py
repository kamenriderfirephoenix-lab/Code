...
class Player:
    ...
    def __init__(self,name='Hero',class_name='Warrior'):
        ...
        if class_name == 'Monk':
            ...
            self.intelligence=-1000; self.wisdom=-1000; self.will=-1000; self.constitution=5
        else:
...
class Room:
    ...
    def create_town_layout(self):
        ...
        # === BUILDINGS === 
        # Player's house (top-left) - L-shaped
        self.buildings.append({
            ...
            'pattern': 'brick',
            'interior': [
                {'type': 'rect', 'x': 20, 'y': 20, 'w': 80, 'h': 60, 'color': '#654321'},  # Table
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 20, 'h': 20, 'color': '#8B4513'},  # Chair
            ]
        })
        
        # Library (top-center) - Book shape with shelves
        self.buildings.append({
            'type': 'library',
            'x': 600, 'y': 200,
            ...
            'shape': 'book',
            'interior': [
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 40, 'h': 80, 'color': '#4A3428'},  # Bookshelf
                {'type': 'rect', 'x': 100, 'y': 10, 'w': 40, 'h': 80, 'color': '#4A3428'},  # Bookshelf
            ]
        })
        
        # Blacksmith Forge (top-right) - Anvil shape
        self.buildings.append({
            'type': 'blacksmith',
            'x': 1000, 'y': 200,
            ...
            'shape': 'anvil',
            'interior': [
                {'type': 'oval', 'x': 50, 'y': 40, 'w': 30, 'h': 30, 'color': '#FF4500'},  # Forge fire
                {'type': 'rect', 'x': 80, 'y': 50, 'w': 40, 'h': 20, 'color': '#696969'},  # Anvil
            ]
        })
        
        # Enchanter Tower (center-left) - Tall tower with crystal
        self.buildings.append({
            'type': 'tower',
            'x': 200, 'y': 500,
            'width': 80, 'height': 180,
            ...
            'shape': 'tower',
            'interior': [
                {'type': 'oval', 'x': 20, 'y': 20, 'w': 40, 'h': 40, 'color': '#9370DB'},  # Crystal ball
                {'type': 'rect', 'x': 10, 'y': 100, 'w': 60, 'h': 60, 'color': '#4A3368'},  # Altar
            ]
        })
        
        # Alchemist Shop (center) - Potion bottle shape
        self.buildings.append({
            'type': 'shop',
            'x': 500, 'y': 480,
            'width': 110, 'height': 90,
            ...
            'shape': 'bottle',
            'interior': [
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 30, 'h': 60, 'color': '#1B6B1B'},  # Shelf
                {'type': 'rect', 'x': 70, 'y': 10, 'w': 30, 'h': 60, 'color': '#1B6B1B'},  # Shelf
            ]
        })
        
        # Bakery/Inn (center-right) - Bread loaf shape
        self.buildings.append({
            'type': 'inn',
            'x': 850, 'y': 480,
            'width': 140, 'height': 95,
            ...
            'shape': 'bread',
            'interior': [
                {'type': 'rect', 'x': 50, 'y': 30, 'w': 40, 'h': 40, 'color': '#8B4513'},  # Counter
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 30, 'h': 20, 'color': '#FFD700'},  # Oven
            ]
        })
        
        # Jeweler (bottom-left) - Diamond shape
        self.buildings.append({
            'type': 'shop',
            'x': 220, 'y': 780,
            'width': 100, 'height': 85,
            ...
            'shape': 'diamond',
            'interior': [
                {'type': 'rect', 'x': 30, 'y': 30, 'w': 40, 'h': 30, 'color': '#C25876'},  # Display case
            ]
        })
        
        # General Trader (bottom-center) - Shop front
        self.buildings.append({
            'type': 'shop',
            'x': 600, 'y': 780,
            'width': 120, 'height': 90,
            ...
            'shape': 'store',
            'interior': [
                {'type': 'rect', 'x': 10, 'y': 10, 'w': 40, 'h': 60, 'color': '#36648B'},  # Shelf
                {'type': 'rect', 'x': 70, 'y': 10, 'w': 40, 'h': 60, 'color': '#36648B'},  # Shelf
            ]
        })
    ...
    # === DECORATIONS ===
        # DENSE pine forest around edges - barely see grass
        # Optimized Forest: Larger spread, lower density to prevent lag
        random.seed(42) # Keeps forest consistent
        forest_bounds = [
            {'x': (0, 1400), 'y': (0, 150)},    # Top border
            {'x': (0, 1400), 'y': (950, 1100)}, # Bottom border
            {'x': (0, 180),  'y': (150, 950)},  # Left border
            {'x': (1220, 1400), 'y': (150, 950)} # Right border
        ]

        for area in forest_bounds:
            # Step-based placement prevents clumping
            for tx in range(area['x'][0], area['x'][1], 60): 
                for ty in range(area['y'][0], area['y'][1], 60):
        ...
        lamp_positions = [
            (400, 350),   # Between house and library
            (800, 350),   # Between library and forge
            (400, 650),   # Clear spot mid-left
            (900, 650),   # Clear spot mid-right
        ]
        ...
        # Animated fountain in center
        self.decorations.append({
            'type': 'fountain',
            'x': 700,
            'y': 550,
            'size': 40,
            'water_particles': []  # Will be animated in draw
        })
        ...
        # Inside your Room class, or specifically where town buildings are created:
        self.roads = []
        if self.is_town: # Ensure this only runs for the town (dungeon_id 0)
            # 1. Create a Main Horizontal Road (The "Spine")
            self.roads.append({
                'x1': -400, 'y1': WINDOW_H // 2, 
                'x2': 1350, 'y2': WINDOW_H // 2, 
                'width': 80
            })
            
            # 2. Create a Main Vertical Road (The "Cross")
            self.roads.append({
                'x1': WINDOW_W // 2, 'y1': -400, 
                'x2': WINDOW_W // 2, 'y2': 1050, 
                'width': 80
            })

            # 3. Connect EVERY building to the nearest main road
            for b in self.buildings:
                bx_center = b['x'] + b['width'] // 2
                by_center = b['y'] + b['height'] // 2
                
                # Draw a path from the building to the horizontal spine
                self.roads.append({
        ...
        if self.dungeon_id == 0:
            ...
            for building in self.room.buildings:
                ...
                if (p.x + p.size > bx and p.x - p.size < bx + bw and
                    ...
                    # Not at door - push player back
                    p.x = old_x
                    p.y = old_y
            
            # Much larger world boundaries
            p.x = clamp(p.x, -400, 1240)  # Expanded from 50-1350
            p.y = clamp(p.y, -400, 940)   # Expanded from 50-1050
...
# Draw buildings first (background layer)
            ...
            for building in self.room.buildings:
                ...
                # Shop sign board - like classic RPG shops
                # Wooden sign hanging above door
                sign_board_w = bw * 0.6

                # ADD condition to check if building should have sign:
                if building['type']:
            ...
            for deco in self.room.decorations:
                if deco['type'] == 'tree':
                    ...
                    if tree_style == 'oak':
                        ...
                        self.canvas.create_oval(x-size*0.5, y-size*1.4, x+size*0.5, y-size*0.8, 
                                              fill='#4d7c2f', outline='#1B5E20', width=2)
                    
                    elif tree_style == 'pine':
                        # Pine trunk
                        self.canvas.create_rectangle(x-4, y-size//2, x+4, y+size//2, 
                                                    fill='#654321', outline='')
                        # Triangle pine shape
                        for i in range(3):
                            offset = i * size * 0.4
                            ...
                            self.canvas.create_polygon(
                                x, y - size*1.5 + offset,
                                x - width, y - size*0.8 + offset,
                                x + width, y - size*0.8 + offset,
                                fill='#0d5c0d', outline='#0a4a0a', width=2
                            )
                    
                    elif tree_style == 'birch':
                        # White birch trunk with black marks
                        self.canvas.create_rectangle(x-5, y-size//2, x+5, y+size//2, 
                                                    fill='#f5f5dc', outline='#d3d3d3', width=2)
                        # Black bark marks
                        for mark_y in range(int(y-size//2), int(y+size//2), 15):
                            self.canvas.create_line(x-5, mark_y, x+5, mark_y, 
                                                  fill='#2F4F4F', width=2)
                        # Leafy top
                        self.canvas.create_oval(x-size*0.8, y-size*1.6, x+size*0.8, y-size*0.5, 
                    ...
                    # REALISTIC water spray particles - physics-based
                    current_time = time.time()
                    for i in range(12):
                        # Each particle has its own phase/timing
                        phase = (current_time * 2 + i * 0.5) % 3.0  # 3 second cycle
                        
                        # Launch angle (spray outward in circle)
                        angle = (i / 12) * 2 * math.pi
                        
                        # Parabolic trajectory
                        if phase < 1.5:  # Rising
                            height = pillar_height + phase * 25
                            dist = phase * 15
                            opacity = 1.0
                        else:  # Falling
                            fall_time = phase - 1.5
                            height = pillar_height + 37.5 - fall_time * 30
                    ...
                    for i in range(8):
                        angle = (i / 8) * 2 * math.pi
                        splash_dist = 25 + math.sin(current_time * 3 + i) * 3
                        sx = x + math.cos(angle) * splash_dist
                        sy = y + math.sin(current_time * 3 + i) * 2
                        self.canvas.create_oval(sx-1.5, sy-1.5, sx+1.5, sy+1.5, 
                                               fill='white', outline='')
